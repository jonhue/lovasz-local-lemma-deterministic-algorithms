...
\centering\input{figures/witness_dag_final}\spadding
...
\begin{enumerate}
    ...
    Need to ensure that the effect of perturbing $p$ is small enough!\pause\ \follows for $\tau^\epsilon \leq \epsw(\U)^{-1}$, we have $\w(\U) < 1$\pause\ \follows $\taumax$.\pause
    ...
    \g{Example}\quad $\tau = \only<12-13>{2^0 = 1}\only<14>{2^{-1} = \nicefrac{1}{2}}\only<15>{2^{-2} = \nicefrac{1}{4}}\only<16->{2^{-3} = \nicefrac{1}{8}}$.\vspace{0.25em}
    
    \onslide<17->{\follows $\tau \geq \frac{1}{2} \taumax$}\spadding
    
    \onslide<18->{Are $\U$ and $\L$ of polynomial size?}
    \end{column}
