...
\begin{frame}{Setting}
Independent $\Sigma$-valued coordinates $X_1, \dots, X_n$ with law $\law$.
``Bad-events'' $\B = \{B_1, \dots, B_m\}$, each a boolean function of some subset of coordinates $\var{B_i} \subseteq \{X_1,\dots,X_n\}$.
...
If for any $i$, $\law(B_i) \leq \pmax$ and $B_i$ affects at most $d$ bad-events, then $e \pmax d \leq 1$ implies $\law(\text{all $B_i$ avoided}) > 0$.
\end{theorem}\pause\spadding

For \g{$k$-SAT}, $\law(B_i) = 2^{-k}$ \follows satisfiable if any literal appears in $d \leq \nicefrac{2^k}{e}$ clauses!\pause\ How to find a satisfying assignment?
\end{frame}
...
\begin{algorithm}[H]
    ...
    \While{some bad-event is true on $X$}{
        ...
        For each $i \in \var{B}$, draw $X_i$ from its distribution in $\law$\;
    }
\end{algorithm}\pause
...
\onslide<13->{\follows configuration at step $t$ is drawn according to $\law$!}
\end{frame}
...
\onslide<6->{For a threshold $\tau \in [0,1]$, \begin{itemize} % TODO: L & U are subsets of collectible witness DAGs
    \item let $\L \subseteq \G$ be the set of \b{likely} witness DAGs, $\w(G) \geq \tau$\only<7->{;
    \item let $\U \subseteq \G$ be the set of \b{(most likely) unlikely} witness DAGs, $\w(G) < \tau$ such that all strict prefixes are likely.}
\end{itemize}}

\onslide<8->{\underline{Note}: Fixing resampling table $R$, if $\compat{\U}{R} = \emptyset$, then $\compat{\G}{R} \subseteq \compat{\L}{R}$.}
\end{column}
\begin{column}{.36\textwidth}
\centering\input{figures/likely_unlikely_wdags}
\flushright\small $G$, suppose $\law(B_i) = \nicefrac{1}{2}$.
\end{column}
\end{columns}\spadding

\onslide<9->{\follows need to trade $\tau$!}
\end{frame}
...
\begin{enumerate}
    \item What is the largest $\tau$ guaranteeing $\w(\U) < 1$?\pause
    ...
    \g{Example}\quad $\tau = \only<11-12>{2^0 = 1}\only<13>{2^{-1} = \nicefrac{1}{2}}\only<14>{2^{-2} = \nicefrac{1}{4}}\only<15->{2^{-3} = \nicefrac{1}{8}}$.\vspace{0.25em}
    
    \onslide<16->{\follows $\tau \geq \frac{1}{2} \taumax$}\spadding
    
    \onslide<17->{Are $\U$ and $\L$ of polynomial size?}
    \end{column}
...
\begin{enumerate}
    ...
    \onslide<5->{But $G - B$ is not necessarily single-sink! Its \textcolor{orange}{sinks} are an independent set adjacent to $B$ in $G$.}
    \end{column}
    \begin{column}{0.1\textwidth}
    \centering\vspace{-2em}\input{figures/likely_unlikely_wdags_removing_sink}
    \end{column}
    \begin{column}{0\textwidth}
    \end{column}
