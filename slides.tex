...
``Bad-events'' $\B = \{B_1, \dots, B_m\}$, each a boolean function of some subset of coordinates $\var{B_i} \subseteq \{X_1,\dots,X_n\}$.\pause
...
\begin{itemize}
    ...
    \underline{Note}: $\Pr[R \sim D]{\text{$G$ \& $R$ compatible}} \pause= \prod_{B \in G} \Pr{B} \pause\eqdef w(G)$.
...
\only<6->{For a threshold $\tau$, \begin{itemize}
    \item let $\L$ be the set of \b{likely} witness DAGs, $w(G) \geq \tau$\only<7->{;
    \item let $\U$ be the set of \b{(most likely) unlikely} witness DAGs, $w(G) < \tau$ such that all prefixes are likely.}
\end{itemize}}
...
\flushright\small $G$, suppose $\Pr{B_i} = \nicefrac{1}{2}$.
\end{column}
\end{columns}\spadding
...
\follows $\O{|\U| \cdot n |\L| \cdot |\Sigma| \cdot |\L| T}$\pause, where $T$ is the runtime of computing conditional probabilities of bad-events given a partial resampling table.\pause\par
