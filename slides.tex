...
\centering\only<-4>{\input{figures/witness_dag}}\only<5->{\input{figures/witness_dag_extended}}
\end{column}
\begin{column}{.3\textwidth}
\b{Witness DAG} $\hat{G}$
...
\begin{enumerate}
    ...
    Need to ensure that the effect of perturbing $p$ is small enough!\pause\ \follows for $\tau^\epsilon \leq \epsw(\U)^{-1}$, we have $\w(\U) < 1$\pause\ \follows $\taumax$.
